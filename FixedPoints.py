...
class FixedPoints:
    ...
    def __init__(self,
        ...
        Optional args:
            ...
            cond_id: [n,] numpy array with cond_id[i] indicating the condition ID corresponding
            to inputs[i].
    ...
    def __setitem__(self, index, fps):
        ...
        assert isinstance(fps, FixedPoints),f"fps must be a FixedPoints object but was {type(fps)}."
    ...
    def __eq__(self, other):
        result = True
        if isinstance(other, FixedPoints):
            for attr in (self._data_attrs + ['n', 'n_states', 'n_inputs']):
                data_self = getattr(self, attr)
                data_other = getattr(other, attr)
                if data_self is None and data_other is None:
                    result = result and True
                elif ((data_self is None) and (data_other is not None)) or \
                        ((data_self is not None) and (data_other is None)):
                    result = result and False
                else:
                    result = result and np.array_equal(data_self, data_other, equal_nan=True)
            for attr in self._nonspecific_attrs:
                result = result and (getattr(self, attr) == getattr(other, attr))
        else:
            result = False
        return result
